...
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, _) in enumerate(dataloader):
        ...
        z = Variable(Tensor(np.random.normal(0, 1, (1, opt.latent_dim)))).cuda()
        real_z = Variable(Tensor(np.random.normal(0, 1, (imgs.size(0), opt.latent_dim)))).cuda()
        ...
        loss_DT = -torch.mean(discriminator_T(real_z)) + torch.mean(discriminator_T(z))
        ...
        if i % opt.n_critic == 0:
            ...
            loss_T = -torch.mean(discriminator_T(z))
            ...
            print ("[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [DT loss: %f] [T loss: %f]" % (epoch, opt.n_epochs,
                                                            batches_done % len(dataloader), len(dataloader),
                                                            loss_D.item(), loss_G.item(), loss_DT.item(), loss_T.item()))
