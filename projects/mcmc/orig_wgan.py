...
os.makedirs('images_orig_wgan', exist_ok=True)
...
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, _) in enumerate(dataloader):
        ...
        if i % opt.n_critic == 0:
            ...
            # Adversarial loss
            loss_G = -torch.mean(mcmc.discriminator(gen_imgs))

            # Reconstruction loss

            loss_BCE = F.binary_cross_entropy(gen_imgs, real_imgs)

            loss = loss_BCE + loss_G

            loss.backward()
            optimizer_G.step()
            optimizer_E.step()

            print ("[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [BCE loss: %f]" % (epoch, opt.n_epochs,
                                                            batches_done % len(dataloader), len(dataloader),
                                                            loss_D.item(), loss_G.item(), loss_BCE.item()))

        if batches_done % opt.sample_interval == 0:
            save_image(gen_imgs.data[:100], 'images_orig_wgan/%d.png' % batches_done, nrow=10, normalize=False)
        batches_done += 1
